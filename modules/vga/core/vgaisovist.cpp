...
bool VGAIsovist::run(Communicator *comm) {
    ...
    BSPNode bspRoot = makeBSPtree(comm, m_map.getDrawingFiles());
    ...
    for (size_t i = 0; i < m_map.getCols(); i++) {
        for (size_t j = 0; j < m_map.getRows(); j++) {
            ...
            if (m_map.getPoint(curs).filled()) {
                ...
                isovist.setData(attributes, row, m_simple_version);
    ...
    m_map.m_hasIsovistAnalysis = true;
