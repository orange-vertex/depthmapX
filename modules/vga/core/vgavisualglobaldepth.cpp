...
bool VGAVisualGlobalDepth::run(Communicator *) {
    ...
    for (auto iter = attributes.begin(); iter != attributes.end(); iter++) {
        ...
        m_map.getPoint(pix).m_extent = pix;
    }
        ...
        for (auto currLvlIter = searchTreeAtLevel.rbegin(); currLvlIter != searchTreeAtLevel.rend(); currLvlIter++) {
            ...
            if (p.filled() && p.m_misc != ~0) {
                ...
                if (!p.contextfilled() || currLvlIter->iseven() || level == 0) {
                    p.getNode().extractUnseen(search_tree[level + 1], &m_map);
                    p.m_misc = ~0;
                    if (!p.getMergePixel().empty()) {
                        ...
                        if (p2.m_misc != ~0) {
                            ...
                            p2.getNode().extractUnseen(search_tree[level + 1], &m_map); // did say p.misc
                            p2.m_misc = ~0;
                        }
                    }
                } else {
