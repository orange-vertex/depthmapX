...
void QGraphDoc::SetUpdateFlag(int type, bool modified) {
    switch (type) {
    case NEW_FILE:
        QApplication::postEvent((QObject *)m_mainFrame,
...
void QGraphDoc::OnVGALinksFileImport() {
    ...
    QString infile =
        QFileDialog::getOpenFileName(0, tr("Import Links File"), "", template_string, &selectedFilter, options);
    ...
    if (fileStream.fail()) {
        ...
        try {
            ...
            std::vector<PixelRefPair> newLinks =
                depthmapX::pixelateMergeLines(EntityParsing::parseLines(fileStream, '\t'), currentMap);
...
void QGraphDoc::OnFileImport() {
    ...
    template_string +=
        "Drawing files (*.dxf *.ntf *.gml *.cat *.rt1)\nMapInfo map (*.mif)\nText files (*.txt *.csv)\n";
    ...
    QString selectedFilter;
    QStringList infiles =
        QFileDialog::getOpenFileNames(0, tr("Import Files"), "", template_string, &selectedFilter, options);
    ...
    if (ext == tr("CAT") || ext == tr("DXF") || ext == tr("NTF") || ext == tr("RT1") || ext == tr("MIF") ||
        ext == tr("GML") || ext == tr("")) {
        for (int i = 1; i < infiles.size(); i++) {
            QFilePath filepath(infiles[i]);
            if (filepath.m_ext != ext || !(filepath.m_ext == tr("RT1") || filepath.m_ext == tr("NTF") ||
                                           filepath.m_ext == tr("GML") || filepath.m_ext == tr(""))) {
                QMessageBox::warning(
                    this, tr("Warning"),
                    tr("You have selected more than one file.  Unfortunately, this feature is only currently "
                       "available with NTF, GML and Tiger line files.\nPlease select a single file to import only."),
                    QMessageBox::Ok, QMessageBox::Ok);
                return;
            }
        }
        ...
        if (ok) {
            ...
            if (ext != tr("RT1") && ext != tr("NTF") &&
                ext != tr("GML")) { // ntf, gml & rt1 use filesets (all others use standard file at a time)
        ...
        if (file.fail()) {
            ...
            bool mapParsed = depthmapX::importFile(
                *m_meta_graph, file, comm.get(), filepath.m_name.toStdString(), depthmapX::ImportType::DATAMAP,
...
void QGraphDoc::OnFileExport() {
    ...
    if (ext != tr("MIF") && ext != tr("GRAPH") && ext != tr("NET")) {
        ...
        if (mode >= 3) {
            QMessageBox::warning(
                this, tr("Notice"),
                tr("Sorry, depthmapX only supports exporting VGA, axial and shape data to graph files"),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }

        if (m_meta_graph->write(outfile.toStdString(), METAGRAPH_VERSION, true) !=
            MetaGraph::OK) { // <- true writes current layer only
            QMessageBox::warning(this, tr("Notice"), tr("Sorry, unable to open file for export"), QMessageBox::Ok,
                                 QMessageBox::Ok);
        }
    } else if (ext == tr("NET")) {
        if (mode != 0 && mode != 2) {
            QMessageBox::warning(this, tr("Notice"),
                                 tr("Sorry, depthmapX can only export VGA graphs or shape graphs to Pajek .net files"),
                                 QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        std::ofstream stream(outfile.toLatin1());
        ...
        if (mode >= 3) {
            QMessageBox::warning(
                this, tr("Notice"),
                tr("Sorry, depthmapX currently only supports exporting VGA, axial and shape data to MapInfo tables"),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (midfile.fail() || midfile.bad()) {
            QMessageBox::warning(this, tr("Notice"), tr("Sorry, unable to open associated .mid file for export"),
                                 QMessageBox::Ok, QMessageBox::Ok);
            mode = -1;
        }
...
void QGraphDoc::OnPointmapExportConnectionsAsCSV() {
    ...
    if (!(m_meta_graph->getViewClass() & MetaGraph::VIEWVGA)) {
        QMessageBox::warning(this, tr("Error"), tr("Make sure a Visibility Graph is visible"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return; // No graph to export
    }
...
void QGraphDoc::OnEditGrid() {
    if (m_communicator) {
        QMessageBox::warning(this, tr("Warning"), tr("Please wait, another task is running"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    }
    bool newmap = false;
    ...
    if (m_meta_graph->getPointMaps().empty() || m_meta_graph->getDisplayedPointMap().isProcessed()) {
        ...
        if (QMessageBox::Yes != QMessageBox::question(this, tr("depthmapX"),
...
void QGraphDoc::OnFillPoints(const Point2f &p,
                             int fill_type) // semifilled = 0 (intention to use semifilled steps for part filled)
{
    int state = m_meta_graph->getState();
    if (m_communicator) {
        QMessageBox::warning(this, tr("Notice"), tr("Please wait, another task is running"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    }
    if (~state & MetaGraph::LINEDATA) {
        QMessageBox::warning(this, tr("Notice"),
                             tr("Sorry, line drawing data must be loaded before points may be filled"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    if (~state & MetaGraph::POINTMAPS) {
        QMessageBox::warning(this, tr("Notice"), tr("Please make grid before filling"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    }
    if (m_meta_graph->viewingProcessed()) {
        QMessageBox::warning(this, tr("Notice"),
                             tr("Sorry, once the graph has been constructed, more points may not be added"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnLayerConvert() {
    ...
    if (m_meta_graph->viewingProcessedLines()) {
        ...
        if (type == ShapeMap::AXIALMAP) {
            // note, ShapeMap::ALLLINEMAP is deliberately excluded, as making a segment map from it would overproduce
            // segments:
    ...
    if (dlg.m_mapout != 0 && QDialog::Accepted == dlg.exec()) {
        ...
        if (dlg.m_mapout == MAKELAYER_DRAWING) {
            ...
            m_communicator->SetOption(dlg.m_mapin == MAKELAYER_DATA ? 0 : 1,
...
void QGraphDoc::OnMakeIsovist(const Point2f &seed, double angle) {
    ...
    if (~state & MetaGraph::LINEDATA) {
        QMessageBox::warning(this, tr("Notice"),
                             tr("Sorry, line drawing data must be loaded before an isovist can be constructed"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnToolsIsovistpath() {
    ...
    if (state & MetaGraph::LINEDATA) {
        ...
        if ((view & (MetaGraph::VIEWDATA | MetaGraph::VIEWAXIAL)) != 0 && m_meta_graph->isSelected()) {
            ...
            QMessageBox::warning(
                this, tr("Notice"),
                tr("To get a path you will need some lines for a path.\nTo do so, have an active map which is either "
                   "a data map or line map of some sort.\nNext, select lines or polylines that you wish to turn into "
...
void QGraphDoc::OnToolsAxialMap(const Point2f &seed) {
    ...
    if (~state & MetaGraph::LINEDATA) {
        QMessageBox::warning(this, tr("Notice"),
                             tr("Sorry, line drawing data must be loaded before an axial map can be constructed"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnToolsMakeFewestLineMap() {
    ...
    if (~state & MetaGraph::SHAPEGRAPHS) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("Sorry, all line map must exist in order to construct fewest line map"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
...
void QGraphDoc::OnEditUndo() {
    if (!m_meta_graph->canUndo()) {
        QMessageBox::warning(this, tr("Warning"), tr("Sorry, no undo available for this map"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnEditClear() {
    ...
    if (editable == MetaGraph::NOT_EDITABLE) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("Cannot delete: the geometry forming this graph cannot be edited."), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    } else if (editable == MetaGraph::EDITABLE_OFF) {
        QMessageBox::warning(this, tr("Warning"), tr("Cannot delete: this graph is currently uneditable."),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnToolsMakeGraph() {
    ...
    if (!m_meta_graph->viewingUnprocessedPoints()) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("Sorry, you need an unprocessed set of points to make the visibility graph"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnToolsMPD() {
    ...
    if (m_meta_graph->viewingProcessedPoints() ||
        (m_meta_graph->viewingProcessedLines() && m_meta_graph->getDisplayedShapeGraph().isSegmentMap())) {
        if (m_meta_graph->isSelected()) {
...
int QGraphDoc::OnOpenDocument(char *lpszPathName) {
    ...
    case MetaGraph::WARN_BUGGY_VERSION:
        QMessageBox::warning(this, tr("Warning"),
                             tr("this graph was made with a version of depthmapX that contained slight errors"),
                             QMessageBox::Ok, QMessageBox::Ok);
        ret = TRUE;
        break;
    ...
    case MetaGraph::NOT_A_GRAPH:
        QMessageBox::warning(this, tr("Warning"), tr("Unable to open graph: not recognised as a graph file."),
                             QMessageBox::Ok, QMessageBox::Ok);
        break;
    case MetaGraph::DAMAGED_FILE:
        QMessageBox::warning(this, tr("Warning"), tr("Unable to open graph: the graph file is damaged."),
                             QMessageBox::Ok, QMessageBox::Ok);
        break;
    case MetaGraph::DISK_ERROR:
        QMessageBox::warning(this, tr("Warning"),
                             tr("Unable to open graph: an error occurred while trying to read from the disk."),
                             QMessageBox::Ok, QMessageBox::Ok);
        break;
    case MetaGraph::NEWER_VERSION:
        QMessageBox::warning(this, tr("Warning"),
                             tr("Unable to open graph: this graph has been written by a newer version of depthmapX."),
                             QMessageBox::Ok, QMessageBox::Ok);
        break;
    case MetaGraph::DEPRECATED_VERSION:
        QMessageBox::warning(
            this, tr("Warning"),
            tr("Unable to open graph: this is a graph file format not supported by this version of depthmapX."),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
    default: {
        std::string err = dXstring::formatString(ok);
        QMessageBox::warning(this, tr("Warning"), tr("Unable to open graph: error number "), QMessageBox::Ok,
                             QMessageBox::Ok);
    } break;
    }
...
bool QGraphDoc::OnFileSave() {
    ...
    if (newName.isEmpty()) {
        ...
        QFileDialog::Options options = 0;
        QString outfile = QFileDialog::getSaveFileName(0, tr("Save As"), newName,
                                                       tr("Graph file (*.graph)\nAll files (*.*)"), 0, options);
        if (outfile.isEmpty())
            return false;
...
int QGraphDoc::OnSaveDocument(QString lpszPathName) {
    ...
    if (version != -1 && version != METAGRAPH_VERSION) {
        if (QMessageBox::Yes ==
            QMessageBox::question(this, tr("depthmapX"),
                                  tr("This will overwrite the file with the latest graph format, which may not be "
                                     "readable by previous versions of depthmapX.\nDo you want to overwrite it?"),
                                  QMessageBox::Yes | QMessageBox::No, QMessageBox::No))
...
int QGraphDoc::OnSaveDocument(QString lpszPathName, int version) {
    ...
    if (m_meta_graph->getState() & MetaGraph::BUGGY) {
        if (QMessageBox::No == QMessageBox::question(this, tr("depthmapX"),
    ...
    if (ok == MetaGraph::OK) {
        ...
        QMessageBox::warning(this, tr("Warning"), tr("Unable to save graph: is there enough disk space?"),
                             QMessageBox::Ok, QMessageBox::Ok);
    }
...
bool QGraphDoc::OnCloseDocument(int index) {
    if (m_communicator) {
        QMessageBox::warning(this, tr("Warning"), tr("depthmapX is processing, please kill the process to continue"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return false;
    }
    int i;
    ...
    if (modifiedFlag) {
        int result =
            QMessageBox::question(this, tr("depthmapX"), tr("Do you want to save the changes?"),
                                  QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel, QMessageBox::Cancel);
        if (QMessageBox::Yes == result) {
            OnFileSave();
            if (i == VIEW_TYPES) {
                modifiedFlag = false;
                QApplication::postEvent((QObject *)m_mainFrame, new QmyEvent((enum QEvent::Type)FOCUSGRAPH,
                                                                             (void *)this, CONTROLS_DESTROYALL));
            }
            return true;
...
void QGraphDoc::OnPushToLayer() {
    if (m_meta_graph->viewingProcessed()) {
        ...
        if (toplayerclass == MetaGraph::VIEWVGA) {
            ...
            // eek!
            QMessageBox::warning(this, tr("Warning"), tr("No valid map displayed from which to push data"),
                                 QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        // layers to push to:
        ...
        for (i = 0; i < datamaps.size(); i++) {
            if (toplayerclass != MetaGraph::VIEWDATA || i != m_meta_graph->getDisplayedDataMapRef()) {
                names.insert(std::make_pair(std::pair<int, int>(MetaGraph::VIEWDATA, int(i)),
        ...
        for (i = 0; i < m_meta_graph->getPointMaps().size(); i++) {
            ...
            if (toplayerclass != MetaGraph::VIEWVGA) {
                names.insert(
                    std::make_pair(std::pair<int, int>(MetaGraph::VIEWVGA, int(i)),
                                   std::string("Visibility Graphs: ") + m_meta_graph->getPointMaps()[i].getName()));
            }
        }
...
void QGraphDoc::OnAddColumn() {
    CRenameObjectDlg dlg(tr(
        "Column")); // using the constructor without a column name sets rename column dialog to insert column name mode
    bool success = false;
    while (dlg.exec() == QDialog::Accepted && !success) {
        if (dlg.m_object_name.isEmpty()) {
            QMessageBox::warning(this, tr("Warning"), tr("Column name cannot be empty"), QMessageBox::Ok,
                                 QMessageBox::Ok);
        } else {
            ...
            for (int i = 0; i < tab.getNumColumns(); i++) {
                if (tab.getColumnName(i) == dlg.m_object_name.toStdString()) {
                    QMessageBox::warning(
                        this, tr("Notice"),
                        tr("Sorry, another column already has this name, please choose a unique column name"),
                        QMessageBox::Ok, QMessageBox::Ok);
                    found = true;
                    break;
                }
...
bool QGraphDoc::ReplaceColumnContents(PointMap *pointmap, ShapeMap *shapemap, int col) {
    ...
    if (pointmap != NULL) {
        ...
        QMessageBox::warning(this, tr("Warning"),
                             tr("Replace column contents requires either a shape map or visibility graph"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return false;
    }
    ...
    while (error && QDialog::Accepted == dlg.exec()) {
        ...
        if (n == 0) {
            ...
            if (!proggy.parse(stream)) {
                QString msg =
                    QString("There was an error parsing your formula:\n\n") + proggy.getLastErrorMessage().c_str();
                ...
                if (!retvar) {
                    QString msg =
                        QString("There was an error running your formula:\n\n") + proggy.getLastErrorMessage().c_str();
                    QMessageBox::warning(this, tr("Warning"), msg, QMessageBox::Ok, QMessageBox::Ok);
                    error = true;
                }
            }
...
void QGraphDoc::OnEditSelectToLayer() {
    if ((m_meta_graph->getViewClass() & (MetaGraph::VIEWAXIAL | MetaGraph::VIEWDATA)) && m_meta_graph->isSelected()) {

        CRenameObjectDlg dlg(
            "Layer"); // note, without specifying existing layer name, this defaults to "New layer" behaviour
        if (QDialog::Accepted == dlg.exec()) {
...
void QGraphDoc::OnRemoveColumn() {
    ...
    if (col == -1 || col == -2 || m_meta_graph->isAttributeLocked(col)) {
        QMessageBox::warning(this, tr("Warning"), tr("Cannot remove locked column"), QMessageBox::Ok, QMessageBox::Ok);
               ...
               QMessageBox::question(this, tr("depthmapX"),
                                     tr("Are you sure you want to delete the currently displayed column?"),
                                     QMessageBox::Yes | QMessageBox::No, QMessageBox::No)) {
...
void QGraphDoc::OnToolsAxialConvShapeMap() {
    if (m_meta_graph->getDataMaps().empty()) {
        QMessageBox::warning(this, tr("Warning"), tr("No data source layers for unlink points"), QMessageBox::Ok,
                             QMessageBox::Ok);
        return;
    }
...
void QGraphDoc::OnToolsLineLoadUnlinks() {
    // Get file from user
    QFileDialog::Options options = 0;
    QString outfile = QFileDialog::getOpenFileName(0, tr("Import Unlink IDs"), "",
                                                   tr("Text files (*.txt)\nAll files (*.*)"), 0, options);
    if (outfile.isEmpty())
        return;
    ...
    if (stream.fail()) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("There was an error opening the file.\nPlease check the file is not already open"),
                             QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (dlg.exec() == QDialog::Accepted) {
        ...
        if (!m_meta_graph->getDisplayedShapeGraph().unlinkShapeSet(stream, dlg.m_attribute)) {
            QMessageBox::warning(this, tr("Warning"),
                                 tr("There was an error reading the unlink file.\ndepthmapX is expecting a tab "
                                    "delimited set of unlink IDs"),
                                 QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        m_meta_graph->setViewClass(MetaGraph::SHOWAXIALTOP);
    ...
    if (m_meta_graph && m_meta_graph->viewingShapes()) {
        ...
        if (QDialog::Accepted == dlg.exec()) {
            ...
            if (viewclass & MetaGraph::VIEWDATA) {
                ...
                QMessageBox::warning(this, tr("Warning"), tr("Cannot perform conversion on this map type"),
                                     QMessageBox::Ok, QMessageBox::Ok);
            }
        }
    }
