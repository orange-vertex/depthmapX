...
void QGraphDoc::OnToolsVSP()
{
   ...
   if (m_meta_graph->viewingProcessed()) {
      if (m_meta_graph->isSelected()) {
         ...
         // This is easy too... too easy... hmm... crossed-fingers, here goes:
         m_communicator = new CMSCommunicator();
         CreateWaitDialog(tr("Calculating shortest paths..."));
         m_communicator->SetFunction( CMSCommunicator::VISUALSHORTESTPATHS );

         m_thread.render(this);
      }
   }
}
void QGraphDoc::OnToolsMSP()
{
   if (m_communicator) {
       QMessageBox::warning(this, tr("Warning"), tr("Please wait, another process is running"), QMessageBox::Ok, QMessageBox::Ok);
      return;
   ...
   if (m_meta_graph->viewingProcessed()) {
      if (m_meta_graph->isSelected()) {
         ...
         m_communicator = new CMSCommunicator();
         CreateWaitDialog(tr("Calculating shortest paths..."));
         m_communicator->SetFunction( CMSCommunicator::METRICSHORTESTPATHS );

         m_thread.render(this);
      }
   }
}
void QGraphDoc::OnToolsASP()
{
   if (m_communicator) {
       QMessageBox::warning(this, tr("Warning"), tr("Please wait, another process is running"), QMessageBox::Ok, QMessageBox::Ok);
      return;
   ...
   if (m_meta_graph->viewingProcessed()) {
      if (m_meta_graph->isSelected()) {
         ...
         m_communicator = new CMSCommunicator();
         CreateWaitDialog(tr("Calculating shortest paths..."));
         m_communicator->SetFunction( CMSCommunicator::ANGULARSHORTESTPATHS );
