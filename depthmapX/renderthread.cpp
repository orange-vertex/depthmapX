    ...
    if (comm) {
        ...
        case CMSCommunicator::IMPORT:
            ...
            QApplication::postEvent(
                pMain, new QmyEvent((enum QEvent::Type)FOCUSGRAPH, (void *)pDoc, QGraphDoc::CONTROLS_LOADDRAWING));
        ...
        case CMSCommunicator::IMPORTMIF:
            ...
            case MINFO_MULTIPLE:
                // BUG
                // QMessageBox::warning(0, tr("Warning"), tr("The imported MIF file contains multiple shapes per
                // object.\n Please note that depthmapX has broken these up, so each shape has one row of attribute
            ...
            case MINFO_HEADER:
                emit showWarningMessage(
                    tr("Warning"), tr("depthmapX had a problem reading the header information in your MIF file."));
                break;
            case MINFO_TABLE:
        ...
        case CMSCommunicator::MAKEDRAWING: {
            ...
            ok =
                pDoc->m_meta_graph->convertToDrawing(comm, comm->GetString().toStdString(), (comm->GetOption(1) == 0));
            if (ok) {
                pDoc->modifiedFlag = true;
            } else {
        ...
        case CMSCommunicator::MAKEUSERMAPSHAPE:
            ok = pDoc->m_meta_graph->convertDataToAxial(comm, comm->GetString().toStdString(),
                                                        (comm->GetOption(0) == 1), (comm->GetOption(1) == 1));
            if (ok) {
                if (comm->GetOption(0) == 0) {
                    // note: there is both a new table and a deleted table, but deleted table leads to a greater
                    // redraw:
        ...
        case CMSCommunicator::MAKEGATESMAP: {
            // note: relies on the fact that make data map from drawing sets option 1 to -1, whereas make data layer
            // from graph it to either 0 or 1
            ...
            ok = pDoc->m_meta_graph->convertToData(comm, comm->GetString().toStdString(), (comm->GetOption(0) == 1),
                                                   shapeMapType, copydata);
            if (ok) {
                if (comm->GetOption(0) == 0) {
                    // note: there is both a new table and a deleted table, but deleted table leads to a greater
                    // redraw:
        ...
        case CMSCommunicator::MAKECONVEXMAP: {
            // note: relies on the fact that make convex map from drawing sets option 1 to -1, whereas make convex map
            // from data sets it to either 0 or 1
            int shapeMapType = ShapeMap::DRAWINGMAP;
            bool copydata = false;
            if (comm->GetOption(1) != -1) {
                shapeMapType = ShapeMap::DATAMAP;
            ...
            ok = pDoc->m_meta_graph->convertToConvex(comm, comm->GetString().toStdString(), (comm->GetOption(0) == 1),
                                                     shapeMapType, copydata);
            if (ok) {
                if (comm->GetOption(0) == 0) {
                    // note: there is both a new table and a deleted table, but deleted table leads to a greater
        ...
        case CMSCommunicator::MAKESEGMENTMAP:
            ...
            ok = pDoc->m_meta_graph->convertAxialToSegment(comm, comm->GetString().toStdString(),
                                                           (comm->GetOption(0) == 1), (comm->GetOption(1) == 1),
                                                           double(comm->GetOption(2)) / 100.0);
            if (ok) {
                if (comm->GetOption(0) == 0) {
                    // note: there is both a new table and a deleted table, but deleted table leads to a greater
                    // redraw:
