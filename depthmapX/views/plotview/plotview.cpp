				...
				if (pDoc->m_meta_graph->viewingProcessed()) {
                    ...
                    size_t yfloor =
                        std::distance(idx_y.m_valuePairs.begin(),
                                      std::lower_bound(idx_y.m_valuePairs.begin(), idx_y.m_valuePairs.end(),
                                                       ValuePair(-1, dataY(hit_point.y() + 2))));
                    size_t yceil = std::distance(idx_y.m_valuePairs.begin(),
                                                 std::upper_bound(idx_y.m_valuePairs.begin(), idx_y.m_valuePairs.end(),
                                                                  ValuePair(-1, dataY(hit_point.y() - 2))));
                    // work out anything near this point...
                    pvecint xkeys;
                    for (size_t i = xfloor; i < xceil; i++) {
                        int index = idx_x.m_valuePairs[i].index;
						xkeys.add(index);
					}
					pvecint finalkeys;
                    for (size_t j = yfloor; j < yceil; j++) {
...
void QPlotView::mouseReleaseEvent(QMouseEvent *e)
{
   ...
   size_t yfloor =
       std::distance(idx_x.m_valuePairs.begin(), std::lower_bound(idx_y.m_valuePairs.begin(), idx_y.m_valuePairs.end(),
                                                                  ValuePair(-1, dataY(m_drag_rect_a.bottom() + 2))));
   size_t yceil =
       std::distance(idx_x.m_valuePairs.begin(), std::upper_bound(idx_y.m_valuePairs.begin(), idx_y.m_valuePairs.end(),
