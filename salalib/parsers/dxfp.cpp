...
DxfLayer *
DxfParser::getLayer(const std::string &layer_name) // const <- removed as m_layers may be changed if DXF is poor
{
...
DxfLineType *
DxfParser::getLineType(const std::string &line_type_name) // const <- removed as m_layers may be changed if DXF is poor
{
...
std::istream &DxfParser::open(std::istream &stream) {
    ...
    while (!stream.eof() && section != _EOF) {
        ...
        case BLOCKS:
            ...
            break;
        case ENTITIES:
            openEntities(stream, token); // I'm adding the token here as before the function was unsafe, but I'm not
                                         // sure reuse of this token is a good idea AT 29-APR-11
            section = UNIDENTIFIED;
            break;
        default:
            stream >> token;
        ...
        case SPLINE:
            ...
            if (spline.parse(token, this)) {
                if (spline.numVertices() > 0) {
                    ...
                    size_t line_count = (spline.getAttributes() & DxfSpline::CLOSED) ? spline.numVertices() - 2
...
bool operator==(const DxfVertex &a, const DxfVertex &b) { return (a.x == b.x && a.y == b.y && a.z == b.z); }
...
DxfVertex DxfEllipse::getVertex(int i, int segments) const {
    ...
    double ynew = c * m_majorAxisEndPoint.y + reverse * m_minorMajorAxisRatio * s * m_majorAxisEndPoint.x;
...
std::istream &operator>>(std::istream &stream, DxfToken &token) {
    ...
    token.size =
        codeInputLine.length() + token.data.length() + 2; // might be missing a few end line characters --- never mind
    return stream;
}
