...
bool MapInfoData::exportFile(std::ostream &miffile, std::ostream &midfile, const PointMap &points) {
    ...
    if (m_bounds.empty()) {
        ...
        sprintf(bounds, "Bounds (%10f, %10f) (%10f, %10f)", points.m_region.bottom_left.x,
                points.m_region.bottom_left.y, points.m_region.top_right.x, points.m_region.top_right.y);
        m_bounds = bounds;
    }
...
bool MapInfoData::exportFile(std::ostream &miffile, std::ostream &midfile, const ShapeMap &map) {
    ...
    if (m_bounds.empty()) {
        ...
        sprintf(bounds, "Bounds (%10f, %10f) (%10f, %10f)", map.getRegion().bottom_left.x,
                map.getRegion().bottom_left.y, map.getRegion().top_right.x, map.getRegion().top_right.y);
        m_bounds = bounds;
    }
        ...
        if (isObjectVisible(map.m_layers, map.getAttributeTable().getRow(AttributeKey(shape.first)))) {
            ...
            if (poly.isPoint()) {
                ...
                miffile << "LINE " << poly.getLine().start().x << " " << poly.getLine().start().y << " "
                        << poly.getLine().end().x << " " << poly.getLine().end().y << std::endl;
                miffile << "    PEN (1,2,0)" << std::endl;
...
bool MapInfoData::exportPolygons(std::ostream &miffile, std::ostream &midfile,
    ...
    if (m_bounds.empty()) {
        char bounds[256];
        sprintf(bounds, "Bounds (%10f, %10f) (%10f, %10f)", region.bottom_left.x, region.bottom_left.y,
...
void MapInfoData::writetable(std::ostream &miffile, std::ostream &midfile, const AttributeTable &attributes,
    ...
    */
    ...
    std::sort(indices.begin(), indices.end(),
              [&](size_t a, size_t b) { return attributes.getColumnName(a) < attributes.getColumnName(b); });
