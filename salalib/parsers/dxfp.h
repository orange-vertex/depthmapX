...
class DxfCircle : public DxfEntity, public DxfRegion {
  ...
  public:
    ...
    void reflect(double x, double y);
    void rotate(const DxfVertex &base_vertex, double angle) { DxfRegion::rotate(base_vertex, angle); }
    void translate(const DxfVertex &translation) {
        m_centre.translate(translation);
        DxfRegion::translate(translation);
    }
...
class DxfParser {
  ...
  public:
    ...
    void openEntities(std::istream &stream, DxfToken &token,
                      DxfBlock *block = NULL); // cannot have a default token: it's a reference.  Removed default to
                                               // DxfToken() AT 29.04.11
    //
    const DxfVertex &getExtMin() const;
    const DxfVertex &getExtMax() const;
    DxfLayer *getLayer(
        const std::string &layer_name); // const; <- removed as will have to add layer when DXF hasn't declared one
