...
// Akira Fujimotot, Takayuki Tanaka, and Kansei Iwata. ARTS: Accelerated ray-tracing system.  IEEE Computer Graphics
// and Applications, 6(4):16--26, April 1986
...
void Isovist::makeit(BSPNode *root, const Point2f &p, const QtRegion &region, double startangle, double endangle) {
    ...
    for (; curr != m_blocks.end(); ++curr) {
        ...
        if (curr != m_blocks.begin() && !approxeq(prev->endpoint, curr->startpoint, tolerance)) {
            ...
            // (as the point will not move between isovists, so can record *which* occlusion this is, and spot novel
            // ones)
