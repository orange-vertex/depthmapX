...
namespace EntityParsing {
    ...
    Point2f parsePoint(const std::string &point, char delimiter) {
        ...
        if (strings.size() != 2) {
            std::stringstream message;
            message << "Badly formatted point data, should be <number>" << delimiter << "<number>, was " << point
                    << std::flush;
    ...
    std::vector<IsovistDefinition> parseIsovists(std::istream &stream, char delimiter) {
        ...
        if (strings.size() < 2) {
            throw EntityParseException("Badly formatted header (should contain x, y, can also have angle and "
                                       "viewangle for partial isovists)");
        }
        ...
        if (xcol == -1 || ycol == -1) {
            throw EntityParseException("Badly formatted header (should contain x and y, might also have angle and "
                                       "viewangle for partial isovists)");
        }
