    ...
    void mergePixelPairs(const std::vector<PixelRefPair> &links, PointMap &currentMap) {
        ...
        for (; iter != end; ++iter) {
            ...
            if (!currentMap.includes(link.a) || !currentMap.getPoint(link.a).filled() ||
                ...
                message << "Line ends not both on painted analysis space (index: " << (iter - links.begin() + 1) << ")"
                        << std::flush;
            ...
            if (currentMap.isPixelMerged(link.a) || currentMap.isPixelMerged(link.b)) {
                ...
                std::stringstream message;
                message << "Link pixel found that is already linked on the map (index: " << (iter - links.begin() + 1)
                        << ")" << std::flush;
                throw depthmapX::InvalidLinkException(message.str().c_str());
            }
            ...
            for (; prevIter != iter; ++prevIter) {
                ...
                if (link.a == prevLink.a || link.b == prevLink.b || link.a == prevLink.b || link.b == prevLink.a) {
                    // one of the cells has already been seen.
                    std::stringstream message;
                    message << "Overlapping link found (index: " << (iter - links.begin() + 1) << ")" << std::flush;
                    throw depthmapX::InvalidLinkException(message.str().c_str());
                }
            }
        }
    ...
    void unmergePixelPairs(const std::vector<PixelRefPair> &links, PointMap &currentMap) {
        ...
        for (; iter != end; ++iter) {
            ...
            if (!currentMap.isPixelMerged(link.a) || !currentMap.isPixelMerged(link.b)) {
                // one of the cells is already on the map
                std::stringstream message;
                message << "Link pixel pair found that is not linked on the map (index: " << (iter - links.begin() + 1)
                        << ")" << std::flush;
                throw depthmapX::InvalidLinkException(message.str().c_str());
            }
    ...
    std::vector<SimpleLine> getMergedPixelsAsLines(PointMap &currentMap) {
        ...
        for (; iter != end; ++iter) {
            ...
            mergedPixelsAsLines.push_back(
                SimpleLine(currentMap.depixelate(pixelPair.first), currentMap.depixelate(pixelPair.second)));
        }
