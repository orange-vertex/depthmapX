...
class SalaObj {
  ...
  public:
    ...
    // Built-in Functions, note, some of the groupings contain other operations (eg., math ops includes assign, and
    // logical ops includes both comparators and logical ops)
    enum Func {
        S_FNULL = 0x00000000,
        S_GROUP = 0xf0000000,
  ...
  public:
    ...
    void uninit() {
        reset();
        type = S_UNINIT;
    } // <- used to uninitialise variables before running program, thus they give nice error messages if used before
      // initialisation
...
class SalaCommand {
  ...
  protected:
    //
    SalaProgram
        *m_program; // information about the running program (in particular, the global variable and error stack)
...
class SalaProgram {
    ...
    //
    // column is stored away from the context, as it's not actually passed to the program itself, just used to update a
    // column
    int m_col;
    ...
    SalaObj m_thisobj;
    //
    bool m_marked; // this is used to tell the program that a node has been "marked" -- all marks are cleared at the
...
inline SalaObj operator+(SalaObj &a, SalaObj &b) {
    ...
    default:
        throw SalaError(std::string("Cannot add ") + a.getTypeIndefArt() + a.getTypeStr() + std::string(" to ") +
                        b.getTypeIndefArt() + b.getTypeStr());
    }
    return SalaObj();
}
inline SalaObj operator-(SalaObj &a, SalaObj &b) {
...
inline SalaObj operator*(SalaObj &a, SalaObj &b) {
    ...
    case SalaObj::S_NUMBER:
        return (a.type == SalaObj::S_INT) ? (double(a.data.i) * b.data.f) : (a.data.f * double(b.data.i));
    default:
        throw SalaError(std::string("Cannot multiply ") + a.getTypeIndefArt() + a.getTypeStr() + std::string(" by ") +
                        b.getTypeIndefArt() + b.getTypeStr());
    }
    return SalaObj();
}
...
inline SalaObj operator/(SalaObj &a, SalaObj &b) {
    ...
    case SalaObj::S_NUMBER:
        return (a.type == SalaObj::S_INT) ? (double(a.data.i) / b.data.f) : (a.data.f / double(b.data.i));
    default:
        throw SalaError(std::string("Cannot divide ") + a.getTypeIndefArt() + a.getTypeStr() + std::string(" by ") +
                        a.getTypeIndefArt() + b.getTypeStr());
    }
    return SalaObj();
}
...
inline bool operator==(SalaObj &a, SalaObj &b) {
    ...
    default:
        throw SalaError(std::string("Cannot compare ") + a.getTypeIndefArt() + a.getTypeStr() + std::string(" with ") +
                        b.getTypeIndefArt() + b.getTypeStr() + std::string(" using '=='"));
    }
    return false;
}
...
struct SalaMemberFuncLabel : public SalaFuncLabel {
    ...
    SalaMemberFuncLabel(SalaObj::Type t = SalaObj::S_NONE, SalaObj::Func f = SalaObj::S_FNULL,
                        const std::string &str = std::string(), const std::string &des = std::string()) {
        type = t;
        func = f;
        name = str;
        desc = des;
