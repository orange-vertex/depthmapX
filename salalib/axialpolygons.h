...
inline bool operator>(const AxialVertexKey &a, const AxialVertexKey &b) {
    return (
        ...
        (a.m_ref_key == b.m_ref_key && (a.m_ref_a > b.m_ref_a || (a.m_ref_a == b.m_ref_a && a.m_ref_b > b.m_ref_b))));
}
inline bool operator<(const AxialVertexKey &a, const AxialVertexKey &b) {
    return (
        a.m_ref_key < b.m_ref_key ||
...
class AxialPolygons : public SpacePixel {
  ...
  public:
    ...
    void makeAxialLines(std::set<AxialVertex> &openvertices, std::vector<Line> &lines, KeyVertices &keyvertices,
                        std::vector<PolyConnector> &poly_connections, std::vector<RadialLine> &radial_lines);
