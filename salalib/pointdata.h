...
class PointMap : public PixelBase {
  ...
  public:
    ...
    PointMap(PointMap &&other)
        : m_parentRegion(std::move(other.m_parentRegion)), m_drawingFiles(std::move(other.m_drawingFiles)),
          m_points(std::move(other.m_points)), m_attributes(std::move(other.m_attributes)),
    ...
    Point &getPoint(const PixelRef &p) { return m_points(static_cast<size_t>(p.y), static_cast<size_t>(p.x)); }
    ...
    void requireIsovistAnalysis() {
        if (!m_hasIsovistAnalysis) {
            throw depthmapX::PointMapException(depthmapX::PointMapExceptionType::NO_ISOVIST_ANALYSIS,
