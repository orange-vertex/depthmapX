...
// adds any axial lines from this point to the list of lines, adds any unhandled visible vertices it finds to the
// openvertices list axial lines themselves are added to the lines list - the axial line is only there to record the
// key vertices that comprise the line
        ...
        if (possible || stubpossible) {
            ...
            if (!intersect_exclude(line)) {
                ...
                if (next_vertex.m_initialised &&
                    std::find(m_handled_list.begin(), m_handled_list.end(), next_vertex) == m_handled_list.end()) {
                    openvertices.insert(next_vertex); // <- note, add ignores duplicate adds (each vertex tends to be
                                                      // added multiple times before this vertex is handled itself)
                    ...
                    if (m_vertex_polys[vertex.m_ref_key] !=
                        ...
                        RadialLine radialshort(next_vertex, shortline_segend, vertex.m_point, next_vertex.m_point,
                    ...
                    if (m_vertex_polys[vertex.m_ref_key] !=
                        ...
                        RadialLine radialshort(vertex, shortline_segend, next_vertex.m_point, vertex.m_point,
