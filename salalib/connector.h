...
struct SegmentData : public SegmentRef {
    ...
    SegmentData(SegmentRef ref, SegmentRef p = SegmentRef(), int sd = 0, float md = 0.0f, unsigned int cv = 0xffffffff)
        : SegmentRef(ref) {
        previous = p;
        segdepth = sd;
        metricdepth = md;
...
// note, these are stored in reverse metric depth order (i.e., metric shorter paths are taken off the end of the list
// first)
