...
void loadSalaProgram() {
    ...
    g_sala_member_funcs.push_back(
        SalaMemberFuncLabel(SalaObj::S_GRAPHOBJ, SalaObj::S_FVALUE, "value", "get attribute value"));
    g_sala_member_funcs.push_back(
        SalaMemberFuncLabel(SalaObj::S_GRAPHOBJ, SalaObj::S_FSETVALUE, "setvalue", "set attribute value"));
    ...
    g_sala_member_funcs.push_back(
        SalaMemberFuncLabel(SalaObj::S_GRAPHOBJ, SalaObj::S_FCONNECTIONS, "connections", "get list of connections"));
...
bool SalaProgram::parse(std::istream &program) {
    ...
    while (!program.eof()) {
        ...
        if (!program.eof()) {
            ...
            if (thiscommand.m_command == SalaCommand::SC_FOR || thiscommand.m_command == SalaCommand::SC_IF ||
    ...
    while (!endloop && !program.eof()) {
        ...
        case '(':
            ...
            if (last == SP_DATA) {
                ...
                // (in the future, we may well want to transfer an object hashed function name to the func stack
                // instead)
        ...
        case '[':
            ...
            // check for pair of open / close brackets: [] or [  ] -- this is a null value or empty list depending on
            // context
            {
                char beta = program.peek();
                while (beta != EOF && beta == ' ') {
        ...
        case ':':
            ...
            if (m_command == SC_FOR || m_command == SC_WHILE || m_command == SC_IF || m_command == SC_ELIF ||
                m_command == SC_ELSE) {
                ...
                while (!program.eof() && alpha != '\n') {
                    ...
                    if (!commentfound) {
                        if (alpha == '#') {
                            ...
                            throw SalaError("'For', 'if', 'else', etc cannot have execution part on same line; insert "
                                            "a new line after ':'",
                                            m_line);
                        }
                    }
        ...
        if (group == SalaObj::S_MATH_OPS) {
            ...
            try {
                ...
                case SalaObj::S_OR:
                    ...
                    // this point if the first half evaluates to true, thus emulating C... but it's in reverse order
                    // too!
                    data = evaluate(pointer, p_obj);
                ...
                case SalaObj::S_TUPLE:
                    ...
                    case SalaObj::S_FEXTEND:
                        if (param.type & SalaObj::S_LIST) {
                            ...
                            throw SalaError("Parameter must be a list not " + param.getTypeIndefArt() +
                                                param.getTypeStr(),
                                            m_line);
                        }
                        data = SalaObj(); // returns none
                ...
                case SalaObj::S_POINTMAPOBJ:
                    ...
                    case SalaObj::S_FVALUE: {
                        ...
                        if (str == "Ref Number") {
                            ...
                            data = SalaObj(
                                table->getRow(AttributeKey(obj.data.graph.node)).getValue(table->getColumnIndex(str)));
                        }
                    } break;
...
SalaObj SalaCommand::connections(SalaObj graphobj, SalaObj param) {
    ...
    // for segment maps it can be 'all' or 'forward' or 'back' (a string -- no parameters is excluded due to potential
    // for errors) for axial maps it must be none
    SalaObj list;
...
int SalaObj::precedence() const {
    int prec = 0;
    if ((type & S_BRACKET) ==
        0) { // preserve bracket on func stack until after close bracket, remember to strip any at end
        switch (func()) {
        case S_ASSIGN:
