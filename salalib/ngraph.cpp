...
bool Node::fullyConnected() {
    // not quite correct -- sometimes at corners you 'see through' the very first connection
    return (m_bins[0].count() && m_bins[4].count() && m_bins[8].count() && m_bins[12].count() && m_bins[16].count() &&
            m_bins[20].count() && m_bins[24].count() && m_bins[28].count());
}
...
bool Bin::containsPoint(const PixelRef p) const {
    for (auto pixVec : m_pixel_vecs) {
        if (m_dir & PixelRef::DIAGONAL) {
            ...
            if (p.row(m_dir) == pixVec.start().row(m_dir) && p.col(m_dir) >= pixVec.start().col(m_dir) &&
...
std::ostream &operator<<(std::ostream &stream, const Bin &bin) {
    ...
    for (auto pixVec : bin.m_pixel_vecs) {
        for (PixelRef p = pixVec.m_start; p.col(bin.m_dir) <= pixVec.end().col(bin.m_dir); p.move(bin.m_dir)) {
            if (++c % 10 == 0) {
                stream << "\n    ";
            }
            stream << p << ",";
