...
bool ShapeGraph::read(std::istream &stream) {
    ...
    // note that keyvertexcount and keyvertices are different things! (length keyvertices not the same as
    // keyvertexcount!)
...
void ShapeGraph::makeSegmentMap(std::vector<Line> &lines, std::vector<Connector> &connectors, double stubremoval) {
    // the first (key) pair is the line / line intersection, second is the pair of associated segments for the first
    // line
        ...
        for (auto breaksIter = breaks.begin(); breaksIter != breaks.end();) {
            ...
            for (size_t j = 0; j < keylist.size(); j++) {
                ...
                if (keylist[j] < (int)i) {
                    ...
                    // to be added later
                    segmentlist.insert(
                        std::make_pair(OrderedIntPair(i, keylist[j]), std::pair<int, int>(seg_a, seg_b)));
                }
            }
            if (seg_a != -1 && seg_b != -1) {
