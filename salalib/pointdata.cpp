...
PointMap::PointMap(const QtRegion &parentRegion, const std::vector<SpacePixelFile> &drawingFiles,
    ...
    : m_parentRegion(&parentRegion), m_drawingFiles(&drawingFiles), m_points(0, 0), m_attributes(new AttributeTable()),
...
bool PointMap::setGrid(double spacing, const Point2f &offset) {
    ...
    m_bottom_left = Point2f(m_parentRegion->bottom_left.x + m_offset.x, m_parentRegion->bottom_left.y + m_offset.y);

    m_region = QtRegion(Point2f(m_bottom_left.x - m_spacing / 2.0, m_bottom_left.y - m_spacing / 2.0),
...
bool PointMap::clearPoints() {
    ...
    // each is a slight variation (saves a little time when there's a single selection as opposed to a compound
    // selection someday clean up
    ...
    for (auto &p : m_points) {
        if (p.m_misc == m_undocounter) {
            if (p.m_state & Point::FILLED) {
                ...
                p.m_misc =
                    0; // probably shouldn't set to 0 (can't undo)  Eventually will implement 'redo' counter as well
                m_filled_point_count--;
...
void PointMap::outputMergeLines(std::ostream &stream, char delim) {
    ...
    for (size_t i = 0; i < m_merge_lines.size(); i++) {
        ...
        stream << li.start().x << delim << li.start().y << delim << li.end().x << delim << li.end().y << std::endl;
    }
}
...
Line PointMap::getNextMergeLine() const {
    if (curmergeline < (int)m_merge_lines.size()) {
        return Line(depixelate(m_merge_lines[curmergeline].a), depixelate(m_merge_lines[curmergeline].b));
    }
    return Line();
}
...
PafColor PointMap::getPointColor(PixelRef pixelRef) const {
    ...
    if (state & Point::HIGHLIGHT) {
        ...
        if (state & Point::FILLED) {
            if (m_processed) {
                return dXreimpl::getDisplayColor(AttributeKey(pixelRef), m_attributes->getRow(AttributeKey(pixelRef)),
...
bool PointMap::read(std::istream &stream) {
    ...
    for (size_t j = 0; j < m_cols; j++) {
        ...
        for (size_t k = 0; k < m_rows; k++) {
            ...
            pnt.m_state &=
                (Point::EMPTY | Point::FILLED | Point::MERGED | Point::BLOCKED | Point::CONTEXTFILLED | Point::EDGE);
    ...
    for (size_t i = 0; i < m_cols; i++) {
        ...
        for (size_t j = 0; j < m_rows; j++) {
            ...
            if (getPoint(curs).getState() & Point::FILLED) {
                ...
                sparkPixel2(curs, 1, maxdist); // make flag of 1 suggests make this node, don't set reciprocral process
                                               // flags on those you can see maxdist controls how far to see out to
                ...
                if (comm) {
                    if (qtimer(atime, 500)) {
                        if (comm->IsCancelled()) {
                            tagState(false); // <- the state field has been used for tagging visited nodes... set back
                                             // to a state variable
...
bool PointMap::sieve2(sparkSieve2 &sieve, std::vector<PixelRef> &addlist, int q, int depth, PixelRef curs) {
    ...
    for (auto iter = sieve.m_gaps.begin(); iter != sieve.m_gaps.end(); ++iter) {
        ...
        for (int ind = (int)ceil(iter->start * (depth - 0.5) - 0.5);
            ...
            PixelRef here = PixelRef(curs.x + (q % 2 ? x : -x), curs.y + (q <= 1 || q >= 6 ? y : -y));

            if (includes(here)) {
                hasgaps = true;
                // centre gap checks to see if the point is blocked itself
                bool centregap = (double(ind) >= (iter->start * depth) && double(ind) <= (iter->end * depth));
