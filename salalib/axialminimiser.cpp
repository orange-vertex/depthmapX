    ...
    while (removedflag) {
        ...
        for (size_t i = 0; i < m_axialconns.size(); i++) {
            ...
            for (size_t j = 0; j < axa.m_connections.size(); j++) {
                ...
                if (indextob == ii || m_removed[indextob]) { // <- removed[indextob] should never happen as it should
                ...
                if (axa.m_connections.size() <= axb.m_connections.size()) {
                    ...
                    for (size_t axai = 0, axbi = 0; axai < axa.m_connections.size() && axbi < axb.m_connections.size();
                         axai++, axbi++) {
                        ...
                        while (axbi < axb.m_connections.size() &&
                               (axb.m_connections[axbi] == ii || axa.m_connections[axai] > axb.m_connections[axbi])) {
                            axbi++;
                        }
    ...
    for (size_t y = 0; y < m_axialconns.size(); y++) {
        if (!m_removed[y] && !m_vital[y]) {
            ...
            m_vps[livecount].value2 =
                (float)depthmapX::getMapAtIndex(m_alllinemap->m_shapes, y)->second.getLine().length();
            livecount++;
        }
    }
        ...
        if (!presumedvital) {
            ...
            for (auto affectedconnection : affectedconnections) {
                if (!m_removed[affectedconnection]) {
                    ...
                    if (connections.size() <=
                        2) { // <- note number of connections includes itself... so you and one other
                        presumedvital = true;
                        break;
                    }
                }
            ...
            for (int diva : divisorsa) {
                ...
                for (int divb : divisorsb) {
                    ...
                    if (std::find(connections.begin(), connections.end(), divb) != connections.end()) {
                        // as a further challenge, they must link within in the zone of interest, not on the far side
                        // of it... arg!
                        Point2f p =
                            intersection_point(axiallines[diva].getLine(), axiallines[divb].getLine(), TOLERANCE_A);
